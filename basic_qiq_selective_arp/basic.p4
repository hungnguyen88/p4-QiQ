...
control MyIngress(inout headers hdr,
    ...
    action selective3_qiq_access(egressSpec_t port, bit<3> PRI, bit<1> CFI, bit<12>  VID) {
        standard_metadata.egress_spec = port;
        hdr.vlan_qiq.setValid();
        hdr.vlan_qiq.PRI = PRI;
        hdr.vlan_qiq.CFI = CFI;
    ...
    action selective3_qiq_release(egressSpec_t port, bit<12> VID) {
        standard_metadata.egress_spec = port;
        hdr.vlan.VID = VID;
        hdr.ipv4.ttl = hdr.ipv4.ttl - 1;        
        hdr.vlan_qiq.setInvalid();
    ...
    action selective3_qiq_forward(egressSpec_t port) {
        standard_metadata.egress_spec = port;
        hdr.ipv4.ttl = hdr.ipv4.ttl - 1;
    }

    action selective3_qiq_arp(egressSpec_t port, macAddr_t srcAddr, macAddr_t dstAddr) {
        standard_metadata.egress_spec = port;
   ...
   table vlan_exact {
    ...
    table selective3_qiq_access_exact {
        key = {
            hdr.vlan.VID: exact;
            hdr.ethernet.dstAddr: exact;
        }
        actions = {
            selective3_qiq_access;
            drop;
            NoAction;
        }
        size = 1024;
    ...
    table selective3_qiq_exact {
        key = {
            hdr.vlan_qiq.VID: exact;
            hdr.ethernet.dstAddr: exact;
        }
        actions = {
            selective3_qiq_release;
            selective3_qiq_forward;
            drop;
            NoAction;
        }
        size = 1024;
    ...
    table selective3_qiq_arp_exact {
        key = {
            hdr.vlan_qiq.VID: exact;
            hdr.ipv4.dstAddr: lpm;
        }
    ...
    if (hdr.vlan.isValid()&&!hdr.vlan_qiq.isValid()&&(vlan_access_mode==0)) {
            ...
            selective3_qiq_access_exact.apply();
        }
    ...
    if (hdr.vlan_qiq.isValid()&&(vlan_access_mode==0)&&(qiq_access_mode==0)) {
            ...
            selective3_qiq_exact.apply();
            selective3_qiq_arp_exact.apply();       
        }
    }
}
